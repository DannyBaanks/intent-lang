...
from intentlang.ir import Concept, Intent, Provenance, Status
...
def prov(**kw):
    ...
    base = {"surface": "copia la foto", "language": "es",
...
def test_usa_una_palabra_DISTINTA_a_la_del_usuario():
    ...
    # Comparacion por PALABRA COMPLETA, no por substring.
    #
    # El assert del operando decia `intent.operand.lemma not in frase`, que es
    # containment de substring. Cuando el round-trip devolvio "fotografia" para
    # el lema "foto" -- una re-lexicalizacion CORRECTA, palabra distinta del
    # mismo synset -- el test fallo, porque "foto" esta contenido dentro de
    # "fotografia". El assert acusaba passthrough donde no lo habia.
    #
    # Antes pasaba por suerte: dependia de cual sinonimo eligiera el synset. Al
    # ordenar los sentidos por frecuencia (lexicon.senses) cambio el elegido y
    # el test se cayo. La fragilidad ya estaba; el cambio solo la mostro.
    palabras = {p.strip(".,:;()¿?¡!").lower() for p in frase.split()}

    assert intent.verb.lemma.lower() not in palabras, (
        f"passthrough de strings en verbo: {frase!r}")
    assert intent.operand.lemma.lower() not in palabras, (
        f"passthrough de strings en operando: {frase!r}")
